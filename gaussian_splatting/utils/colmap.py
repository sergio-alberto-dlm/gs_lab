...
class camera:
    def __init__(self, camera_type,  width, height, fx, fy, cx, cy, k1=0, k2=0, k3=0, k4=0, p1=0, p2=0):
...
class SceneManager:
    ...
    def load_points3D(self):
        ...
        self.point3D_colors = np.vstack((vertex['red'], vertex['green'], vertex['blue'])).T
        self.normals = np.vstack((vertex['nx'], vertex['ny'], vertex['nz'])).T
        self.point3D_errors = np.zeros((len(self.points3D,)))
...
class Parser:
    ...
    def __init__(
        ...
        if not (type_ == 0 or type_ == 1 or type_ == "PINHOLE" or type_ == "SIMPLE_PINHOLE"):
        ...
        if not os.path.isdir(depth_dir):
            ...
            # gather all the filenames under depths/, relative to the directory root
            depth_rel = sorted(_get_rel_paths(depth_dir))
            # build a map: basename → full path
            depth_map = {
                os.path.basename(p): os.path.join(depth_dir, p)
                for p in depth_rel
            }

            # now align with your image_names (they contain basenames, e.g. "0001.png")
            self.depth_paths = []
            missing = []
            for img_name in self.image_names:
                if img_name in depth_map:
                    self.depth_paths.append(depth_map[img_name])
                else:
                    missing.append(img_name)
                    self.depth_paths.append(None)

            if len(missing) > 0:
                print(f"[Parser] warning: no depth file for {len(missing)} images:", missing)
        ...
        if self.load_depths:
            ...
            if self.parser.depth_paths is not None:
                ...
                if depth_path is None:
                    raise ValueError(
                        f"No depth‐map file for {self.parser.image_names[index]}"
                    )
                raw_depth = imageio.imread(depth_path).astype(np.float32)
                # optional: crop & patch‐align your depth to match `image` / K
                # if you applied undistort/crop/patch, apply the same transforms here!
                data["depths"] = torch.from_numpy(raw_depth)
            #############################################################################################
            else:
